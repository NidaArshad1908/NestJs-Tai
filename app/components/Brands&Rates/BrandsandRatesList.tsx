...
const BrandsandRatesList: NextPage = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:4000/brandrateslist/all')
  ...
  const createOrUpdate = (todo: Countries) => {
    fetch('http://localhost:4000/brandrateslist/createOrUpdate', {
      method: 'POST',
      headers: {
  ...
  const deleteTodo = (event: any) => {
    ...
    fetch(`http://localhost:4000/brandrateslist/delete`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
...
export default BrandsandRatesList;
