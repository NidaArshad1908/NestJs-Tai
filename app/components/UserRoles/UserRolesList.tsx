...
const UserRolesList: NextPage = () => {
  const [todos, setTodos] = useState<UsersRoles[]>([]);
  const [selectedTodos, setSelectedTodos] = useState<UsersRoles[]>([]);
  ...
  useEffect(() => {
    fetch('http://localhost:4000/usersrolelist/all')
  ...
  const handleClickOpen = () => {
    setUpdateTitle(selectedTodos[0]?.role);
    setOpenDialog(true);
  };
  ...
  const createOrUpdate = (todo: UsersRoles) => {
    fetch('http://localhost:4000/usersrolelist/createOrUpdate', {
      method: 'POST',
      headers: {
  ...
  const deleteTodo = (event: any) => {
    ...
    fetch(`http://localhost:4000/usersrolelist/delete`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    >
      <ThemeProvider theme={defaultMaterialTheme}>
        <MaterialTable
          ...
          onSelectionChange={(rows: UsersRoles[]) => {
