...
const SaleInvoicesList: NextPage = () => {
  const [todos, setTodos] = useState<SaleInvoices[]>([]);
  const [selectedTodos, setSelectedTodos] = useState<SaleInvoices[]>([]);
  ...
  useEffect(() => {
    fetch('http://localhost:4000/saleinvoiceslist/all')
  ...
  const handleClickOpen = () => {
    setUpdateTitle(selectedTodos[0]?.date);
    setOpenDialog(true);
  };
  ...
  const handleUpdateAndClose = () => {
    let todoToUpdate: SaleInvoices = {
      id: selectedTodos[0]?.id,
      date: UpdateTitle,
      clothamount: UpdateTitle,
      rates: UpdateTitle,
  ...
  const createOrUpdate = (todo: SaleInvoices) => {
    fetch('http://localhost:4000/saleinvoiceslist/createOrUpdate', {
      method: 'POST',
      headers: {
  ...
  const deleteTodo = (event: any) => {
    ...
    fetch(`http://localhost:4000/saleinvoiceslist/delete`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      <ThemeProvider theme={defaultMaterialTheme}>
        <MaterialTable
          ...
          columns={[
            ...
            { title: 'Cloth Amount', field: 'clothamount' },
          ...
          onSelectionChange={(rows: SaleInvoices[]) => {
...
export default SaleInvoicesList;
