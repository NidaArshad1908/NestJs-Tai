...
const TotalSalesList: NextPage = () => {
  const [todos, setTodos] = useState<TotalSales[]>([]);
  const [selectedTodos, setSelectedTodos] = useState<TotalSales[]>([]);
  ...
  useEffect(() => {
    fetch('http://localhost:4000/totalsaleslist/all')
  ...
  const createOrUpdate = (todo: TotalSales) => {
    fetch('http://localhost:4000/totalsaleslist/createOrUpdate', {
      method: 'POST',
      headers: {
  ...
  const deleteTodo = (event: any) => {
    ...
    fetch(`http://localhost:4000/totalsaleslist/delete`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    >
      <ThemeProvider theme={defaultMaterialTheme}>
        <MaterialTable
          ...
          onSelectionChange={(rows: TotalSales[]) => {
