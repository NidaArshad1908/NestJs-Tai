...
const FunctionsList: NextPage = () => {
  const [todos, setTodos] = useState<Functions[]>([]);
  const [selectedTodos, setSelectedTodos] = useState<Functions[]>([]);
  ...
  useEffect(() => {
    fetch('http://localhost:4000/functionslist/all')
  ...
  const handleClickOpen = () => {
    setUpdateTitle(selectedTodos[0]?.functions);
    setOpenDialog(true);
  };
  ...
  const createOrUpdate = (todo: Functions) => {
    fetch('http://localhost:4000/functionslist/createOrUpdate', {
      method: 'POST',
      headers: {
  ...
  const deleteTodo = (event: any) => {
    ...
    fetch(`http://localhost:4000/functionslist/delete`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    >
      <ThemeProvider theme={defaultMaterialTheme}>
        <MaterialTable
          ...
          columns={[
            { title: 'ID', field: 'id' },
            { title: 'Functions', field: 'functions' },
          ...
          onSelectionChange={(rows: Functions[]) => {
