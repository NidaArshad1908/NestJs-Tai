...
const WorkersList: NextPage = () => {
  const [todos, setTodos] = useState<Workers[]>([]);
  const [selectedTodos, setSelectedTodos] = useState<Workers[]>([]);
  ...
  useEffect(() => {
    fetch('http://localhost:4000/workerslist/all')
  ...
  const createOrUpdate = (todo: Workers) => {
    fetch('http://localhost:4000/workerslist/createOrUpdate', {
      method: 'POST',
      headers: {
  ...
  const deleteTodo = (event: any) => {
    ...
    fetch(`http://localhost:4000/workerslist/delete`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    <div style={{ marginRight: -15, marginLeft: -15, marginTop: -15, marginBottom: -15 }}>
      <ThemeProvider theme={defaultMaterialTheme}>
        <MaterialTable
          ...
          onSelectionChange={(rows: Workers[]) => {
